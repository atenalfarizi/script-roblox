-- Magic Loot - Gift by Value (M/B/T + item baru)
...
local Prices = {
	...
	-- baru (dari screenshot) — sesuaikan harganya
...
local pScroll = Instance.new("ScrollingFrame", f)

pScroll.Size = UDim2.new(1, -12, 0, 70)

pScroll.Position = UDim2.new(0, 6, 0, 80)
...
valBox.Size = UDim2.new(1, -12, 0, 30)

valBox.Position = UDim2.new(0, 6, 0, 158)

valBox.BackgroundColor3 = Color3.fromRGB(40, 40, 50)

valBox.PlaceholderText = "Target: 500=500M, 1000=1B"
...
local startBtn = mk(198, "Start Gift by Value", Color3.fromRGB(0, 140, 60))

local scanBtn = mk(232, "Scan Inventory (1x)", Color3.fromRGB(70, 70, 90))

local stopBtn = mk(266, "Stop", Color3.fromRGB(140, 40, 40))
...
local function scanInventory()
	...
	if #found == 0 then
		...
		if #preview > 90 then
			...
			preview = preview:sub(1, 87) .. "..."

		end

		status(string.format(

			"Cache %d item | Total %s\n%s",

			#found, formatVal(total), preview
...
local function runGiftByValue()
	...
	if target <= 0 then status("Isi target (500=500M, 1000=1B)") return end
	...
	for _, it in ipairs(queue) do
		...
		status(string.format(
			...
			it.name, formatVal(it.price), formatVal(gifted), formatVal(target)
...
print("Gift M/B/T + item baru loaded")